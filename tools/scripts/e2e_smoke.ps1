...
function New-TempDataDir() {
  $path = Join-Path $env:TEMP ("kubercoin_e2e_smoke_" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  Write-Section "RPC smoke"
  $count = Invoke-RpcRaw -HostName $TargetHost -Port $RpcPort -ApiKey $ApiKey -Method "getblockcount"
  Assert-RpcOk -Response $count -Method "getblockcount"
  if ($count.result -isnot [int] -and $count.result -isnot [long]) {
    throw "getblockcount did not return an integer"
  }
  Write-Host ("getblockcount={0}" -f $count.result)

  $chainInfo = Invoke-RpcRaw -HostName $TargetHost -Port $RpcPort -ApiKey $ApiKey -Method "getblockchaininfo"
  Assert-RpcOk -Response $chainInfo -Method "getblockchaininfo"
  if (-not $chainInfo.result.bestblockhash) {
    throw "getblockchaininfo missing bestblockhash"
  }

  $bestBlock = Invoke-RpcRaw -HostName $TargetHost -Port $RpcPort -ApiKey $ApiKey -Method "getblock" -Params @($chainInfo.result.bestblockhash)
  Assert-RpcOk -Response $bestBlock -Method "getblock"
  if ($bestBlock.result.hash -ne $chainInfo.result.bestblockhash) {
    throw "getblock returned a hash different from bestblockhash"
  }

  Write-Section "REST smoke"
  ...
  if (-not $info.version -or -not $info.tip) {
    throw "/api/info was missing required fields"
  }

  $genesis = Invoke-RestMethod -Method Get -Uri ("http://{0}:{1}/api/block-by-height/0" -f $TargetHost, $RpcPort) -TimeoutSec 5
  if ($genesis.height -ne 0) {
    throw "/api/block-by-height/0 did not return height 0"
  }

  Write-Host "`nE2E smoke passed" -ForegroundColor Green
  exit 0
} catch {
  Write-Host "E2E smoke failed: $($_.Exception.Message)" -ForegroundColor Red
  exit 1
} finally {
  if ($proc) {
    try {
