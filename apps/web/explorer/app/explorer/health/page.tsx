...
export default function ChainHealthPage() {
  ...
  const [healthMetrics, setHealthMetrics] = useState({
    overall: 0, syncStatus: 0, peerConnectivity: 0, blockPropagation: 0,
    transactionRelay: 0, mempoolHealth: 0, storageHealth: 0, networkLatency: 0,
  });
  const [chainMetrics, setChainMetrics] = useState({
    currentHeight: 0, targetHeight: 0, syncProgress: 0, avgBlockTime: 0,
    targetBlockTime: 600, orphanRate: 0, reorgDepth: 0, maxReorgDepth: 0,
  });
  ...
  const fetchHealth = useCallback(async () => {
    try {
      const [healthRes, infoRes] = await Promise.all([
        fetch('/api/node/health').catch(() => null),
      ...
      const height: number = health?.height ?? info?.height ?? 0;
      const peers: number = health?.peers ?? info?.peers ?? 0;
      const mempoolSize: number = health?.mempool_size ?? info?.mempool_size ?? 0;
      const online = health?.status === 'ok' || info?.status === 'ok';

      const peerScore = online ? Math.min(100, Math.round(peers * 20)) : 0;
      const storageScore = online ? 100 : 0;
      const relayScore = online ? (mempoolSize >= 0 ? 100 : 80) : 0;
      const mempoolScore = online ? Math.min(100, Math.max(0, 100 - Math.round(mempoolSize / 500))) : 0;
      const overall = online
        ? Math.round((peerScore + storageScore + relayScore + mempoolScore) / 4)
        : 0;

      setHealthMetrics({
        overall,
        syncStatus: online ? 100 : 0,
        peerConnectivity: peerScore,
        blockPropagation: online ? 100 : 0,
        transactionRelay: relayScore,
        mempoolHealth: mempoolScore,
        storageHealth: storageScore,
        networkLatency: online ? 95 : 0,
      });
      setChainMetrics(prev => ({
        ...prev,
        currentHeight: height,
        targetHeight: height,
        syncProgress: online ? 100 : 0,
      }));
