...
export function DonutChart({
	...
	const { segments, total } = useMemo(() => {
		...
		const segs = data.map((item, index) => {
			const percentage = t === 0 ? 0 : (item.value / t) * 100;
			const angle = t === 0 ? 0 : (item.value / t) * 360;
			const startAngle = currentAngle;
			const endAngle = currentAngle + angle;
			currentAngle = endAngle;

			const segmentColor = item.color || `hsl(${(index * 360) / data.length}, 70%, 60%)`;
		...
		<div className={styles.container}>
			...
			>
				...
				{(centerLabel || centerValue) && (
					<g>
						<text x={center} y={center - 8} textAnchor="middle" className={styles.centerLabel}>
							{centerLabel}
						</text>
						<text x={center} y={center + 12} textAnchor="middle" className={styles.centerValue}>
							{centerValue}
						</text>
					</g>
				)}
				...
				<div className={styles.legend}>
					{segments.map((segment) => (
						<div key={segment.label} className={styles.legendItem}>
							<div className={styles.legendColor} style={{ background: segment.color }} />
							<span className={styles.legendLabel}>{segment.label}</span>
							<span className={styles.legendValue}>{segment.percentage.toFixed(1)}%</span>
						</div>
					))}
				</div>
			)}
