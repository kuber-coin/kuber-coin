...
export function BarChart({
	...
	return (
		<div className={`${styles.container} ${isHorizontal ? styles.horizontal : ''}`} style={{ height }}>
							...
							{isHorizontal ? (
								<>
									...
									<div className={styles.barContainerHorizontal}>
										...
										>
											{showValues && percentage > 15 && <span className={styles.valueInside}>{item.value}</span>}
										</div>
										{showValues && percentage <= 15 && <span className={styles.valueOutside}>{item.value}</span>}
									</div>
								</>
							) : (
								<>
									<div className={styles.barContainerVertical}>
										...
										>
											{showValues && percentage > 15 && <span className={styles.valueTop}>{item.value}</span>}
										</div>
									</div>
									{showValues && percentage <= 15 && <span className={styles.valueBelow}>{item.value}</span>}
