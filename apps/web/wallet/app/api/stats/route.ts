...
const NODE_URL = process.env.KUBERCOIN_WALLET_API_URL || process.env.KUBERCOIN_RPC_URL || 'http://localhost:8634';
...
export async function GET() {
  try {
    const [infoRes, peersRes] = await Promise.all([
      fetchWithTimeout(`${NODE_URL}/api/info`, { headers: getWalletAuthHeaders() }, 4000),
      fetchWithTimeout(`${NODE_URL}/api/peers`, { headers: getWalletAuthHeaders() }, 4000),
    ]);
